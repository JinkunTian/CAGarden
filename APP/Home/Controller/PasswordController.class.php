...
class PasswordController extends CommonController {
    public function view(){
        ...
        $pw_info=M('public_password')->where(array('pw_id'=>$pw_id))->find(); //查询密码
        if(!$pw_info){
        	$this->error('密码不存在！');
        }
        ...
        $pr_p_info=M('projects')->where(array('pr_id'=>$pw_info['pw_prid']))->select(); //查询密码所在项目信息
        $pw_info['password']=decode($pw_info['password']);
        $pw_info['note']=decode($pw_info['note']);

        $pw_addition=M('public_password_addition')->where(array('original_password_id'=>$pw_id))->select();
    ...
    public function del(){
        if (isset($_GET['pw_id'])) {
            ...
            if($pw_info){
                $a=M('public_password')->where(array('pw_id'=>$pw_info['pw_id']))->delete();
                $b=M('public_password_addition')->where(array('original_password_id'=>$pw_info['pw_id']))->delete();
                if((!($a===false))&&(!($b===false))){
                    $this->success('删除成功！',U('/Home/Index/index',array('pr_id'=>$pw_info['pw_prid'])));
                }else{
                    $this->error('删除失败！');
                }
            }else{
    ...
    public function save(){
        ...
        if(isset($_POST['edit'])){
            ...
            $pass_update=M('public_password')->where(array('pw_id' =>$pw_id))->save($data);
            //保存数据
            if($pass_update===false) {
                $this->error('保存失败，请重试！',U('/Home/Index/index/pr_id/'.I('pr_id')));
            }else{
                M('public_password_addition')->where(array('original_password_id' => $pw_id, ))->delete();
                ...
                if($addition_password['count']>0){
                    ...
                    $addition_update=M('public_password_addition')->addAll($addition_password['password']);
                }
                if($addition_update===false){
                    $this->error('保存失败，请重试！',U('/Home/Index/index/pr_id/'.I('pr_id')));
                }else{
                    $this->success('保存成功',U('/Home/Password/view/pw_id/'.I('pw_id')));
                }       
            }
            ...
            $result=M('public_password')->add($data);
            ...
            if(!($result===false)){
                if($addition_password['count']>0){
                    ...
                    if($addition_update===false){
                        $this->error('添加失败，请重试！',U('/Home/Index/index/pr_id/'.$data['pw_prid']));
                    }else{
                        $this->success('添加成功',U('/Home/Index/index/pr_id/'.$data['pw_prid']));
                    }
                }
                $this->success('添加成功',U('/Home/Index/index/pr_id/'.$data['pw_prid']));
            }else{
